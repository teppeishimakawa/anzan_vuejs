...
export default {
  data: function () {
    return {
      ...
      num_arr1: [require('@/assets/gu.png'), require('@/assets/cho.png'), require('@/assets/pa.png')],
  ...
  computed: {
    ...
    オブジェクトがネストした状態になってしまうから。よって...付ける
    this.$store.state.xxxxをthis.xxxxで呼べるようにするためのもの
    */
  ...
  watch: {
    getsttflg (num, old) {
      ...
      //  sttしたらquestionとcommentをランダムに選択
  ...
  methods: {
    ...
    result (a, b, c) {
      ...
      //  random1:gu:0,cho:1,pa:2
      //  gu出し：勝って cho,負けてpa,引き分けてgu 正解パターンa,b,c: 1 2 0
      //  cho出し：勝って pa, 負けて gu, 引き分けて cho 正解パターンa,b,c: 2 0 1
      //  pa出し：勝って gu, 負けて cho, 引き分けて pa  正解パターンa,b,c: 0 1 2
